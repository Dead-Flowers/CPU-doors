...
class UserDeviceChallengeResponse(Schema):
    challenge_token: str = Field(
        ..., description="Device challenge token issued by the server"
    )


class UserDeviceChallengeSchema(UserDeviceChallengeResponse):
    challenge_token_signature: str = Field(
        ...,
        description="Base-64 encoded, SHA1 of`challenge_token`, signed with device's private key.",
    )
...
def validate_device_challenge(request, body: UserDeviceChallengeSchema) -> UserDevice:
    try:
        ...
        ninja_jwt.exceptions.TokenError,
        ninja_jwt.exceptions.InvalidToken,
        ninja_jwt.exceptions.ValidationError,
    ) as ex:
        raise ChallengeFailedError("Invalid token (%s)" % str(ex))
    try:
        public_key = device.rsa_public_key
        public_key.verify(
            base64.b64decode(body.challenge_token_signature),
            body.challenge_token.encode(),
            padding=PKCS1v15(),
            algorithm=SHA256(),
        )
